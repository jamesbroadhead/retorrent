...
void FormatConfig::formatString(QString& str) const
{
	...
	if (m_strRepEnabled) {
		...
		// TODO: If str contains 'DIR/FILE, mkdir -p DIR, cd DIR, continue 
		// TODO	
		
		if (path->contains( QChar("/"))) {
			str = preparenesteddir(str);
		}
...
/** 
 * Change cwd to bottom dir in a path, creating dirs if necessary
 *
 * @param path path
 */
void FormatConfig::nestedDirectories(QString& path)
{
	if (QDir::mkpath (path)) {
	
	
	while (path->contains( QChar("/") )) {
