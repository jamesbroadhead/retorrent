...
config_paths = [abspath('./'), home_config_dir, '/usr/share/retorrent/']
...
def parse_fileext_details(extra_configdir=''):
    ...
    if not filepath:
        raise EnvironmentError('Could not locate or load %s and cannot operate' % (
            filename,) + 'without it. A default should be in %s' % (config_paths[-1],) +
